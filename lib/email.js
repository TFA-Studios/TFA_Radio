    ...
    '<body style="margin:0; padding:0; background:#DEDCD7; font-family:Arial, Helvetica, sans-serif;">' +
      '<div style="max-width:560px; margin:0 auto; padding:32px 20px;">' +
        '<div style="background:#FFFFFF; border-radius:16px; padding:36px 32px;">' +
          ...
          '<p style="font-size:14px; line-height:1.6; color:#5C5850; margin:0 0 20px;">' +
            esc(companyName) + ' heeft zojuist een radiocommercial-aanvraag ingediend (' + esc(spotLength) + '″' + esc(variationsSummaryLabel(brief)) + ').' +
          '</p>' +
          (dashboardUrl
...
function buildReviewReadyHtml(brief, reviewUrl) {
  const companyName = (brief.companyName && brief.companyName.trim()) ? brief.companyName : 'jouw radiocommercial';
  return (
    '<!doctype html><html lang="nl"><head><meta charset="utf-8"></head>' +
    '<body style="margin:0; padding:0; background:#DEDCD7; font-family:Arial, Helvetica, sans-serif;">' +
      '<div style="max-width:560px; margin:0 auto; padding:32px 20px;">' +
        '<div style="background:#FFFFFF; border-radius:16px; padding:36px 32px;">' +
          ...
          '<p style="font-size:14px; line-height:1.6; color:#5C5850; margin:0 0 22px;">' +
            'De productie van ' + esc(companyName) + ' is klaar voor jouw review. Bekijk hem via de knop hieronder — je kunt feedback achterlaten of direct goedkeuren.' +
          '</p>' +
...
// Sent to the client when a producer starts a new review round (pastes a
// Frame.io link from the dashboard) — the "your round is ready" ping,
// modeled directly on sendResumeLinkEmail above.
export async function sendReviewReadyEmail(brief, reviewUrl) {
  const companyName = (brief.companyName && brief.companyName.trim()) ? brief.companyName : 'je bedrijf';
  const subject = 'Je commercial staat klaar om te bekijken — ' + companyName;
  const html = buildReviewReadyHtml(brief, reviewUrl);
  const to = recipientsFor(brief);
...
export async function sendReviewFeedbackNotification(brief, dashboardUrl) {
  ...
  const html = buildInternalReviewEventHtml(brief, {
    ...
    bodyText: 'heeft feedback achtergelaten op de laatste review-ronde.',
    dashboardUrl,
  });
...
// this workflow ("automatically sent to an admin" on approval).
export async function sendReviewApprovedNotification(brief, { dashboardUrl, frameioLink }) {
  const companyName = (brief.companyName && brief.companyName.trim()) ? brief.companyName : 'Onbekend bedrijf';
  ...
  if (TEAM_NOTIFY_EMAILS.length) {
    ...
    const html = buildInternalReviewEventHtml(brief, {
      ...
      bodyText: 'heeft de laatste review-ronde goedgekeurd.',
      dashboardUrl,
    });
    if (RESEND_API_KEY) {
      try {
      ...
      '<body style="margin:0; padding:0; background:#DEDCD7; font-family:Arial, Helvetica, sans-serif;">' +
        '<div style="max-width:560px; margin:0 auto; padding:32px 20px;">' +
          '<div style="background:#FFFFFF; border-radius:16px; padding:36px 32px;">' +
            ...
            '<p style="font-size:14px; line-height:1.6; color:#5C5850; margin:0 0 20px;">De klant heeft de laatste versie goedgekeurd. Definitieve Frame.io-link:</p>' +
