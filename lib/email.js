...
import { variationsSummaryLabel, variationsCountOf, TONE_LABELS, revisionDisclaimerText, ipRightsDisclaimerText, formatAirDate, parseVariationScripts, finalTrackOf, agencyNameOf } from '../components/flowData';
...
// `opts.trackOverride`, when passed (used by the final-delivery email —
// see buildDeliveryEmailHtml), replaces the "Gekozen muziek" row with a
// single specific track instead of listing all of the client's (up to 3)
// candidates — see finalTrackOf() in components/flowData.js for why. Pass
// `undefined` (the default, i.e. simply omit opts) to keep the normal
// all-candidates behavior used by the client confirmation and team-inbox
// emails.
function buildOverviewFields(brief, opts) {
  ...
  const trackOverridePassed = opts && Object.prototype.hasOwnProperty.call(opts, 'trackOverride');
  const musicValue = trackOverridePassed
    ? (opts.trackOverride ? esc(opts.trackOverride.title) + (opts.trackOverride.artist ? ' (' + esc(opts.trackOverride.artist) + ')' : '') : 'Nog niet vastgelegd welke track definitief gebruikt is')
    : (tracks.length ? tracks.map((t) => esc(t.title) + (t.playlistName ? ' (' + esc(t.playlistName) + ')' : '')).join('<br>') : 'Nog niet gekozen');
  rows.push(['Gekozen muziek', musicValue]);
...
function buildTermsBlock(brief) {
  const items = [
    ...
    ipRightsDisclaimerText(),
    revisionDisclaimerText(brief),
  ];
  return (
      ...
      '<div style="max-width:560px; margin:0 auto; padding:32px 20px;">' +
        '<div style="background:#FFFFFF; border-radius:16px; padding:36px 32px;">' +
          ...
          '<p style="font-size:14px; line-height:1.6; color:#5C5850; margin:0 0 20px;">' +
            'Je commercial-aanvraag is bij ons binnengekomen. We hebben alles hieronder nog even voor je op een rijtje gezet. TFA gaat er nu mee aan de slag en je hoort van je producer zodra er nieuws is.' +
          '</p>' +
          '<table style="width:100%; border-collapse:collapse;">' + rowsHtml + '</table>' +
          scriptBlock +
          variationBlocks +
          disclaimerBlock +
          buildTermsBlock(brief) +
          reviewLink +
...
export async function sendResumeLinkEmail(brief, to, resumeUrl) {
  ...
  const subject = 'Ga verder met je commercial-aanvraag: ' + companyName;
    ...
    '<body style="margin:0; padding:0; background:#DEDCD7; font-family:Arial, Helvetica, sans-serif;">' +
      '<div style="max-width:560px; margin:0 auto; padding:32px 20px;">' +
        '<div style="background:#FFFFFF; border-radius:16px; padding:36px 32px;">' +
          ...
          '<p style="font-size:14px; line-height:1.6; color:#5C5850; margin:0 0 20px;">' +
            esc(companyName) + ' heeft zojuist een commercial-aanvraag ingediend. Overzicht hieronder: plan deze in en wijs een engineer toe in het dashboard.' +
          '</p>' +
          '<table style="width:100%; border-collapse:collapse;">' + rowsHtml + '</table>' +
          scriptBlock +
          variationBlocks +
...
export async function sendConfirmationEmail(brief, appUrl) {
  const companyName = (brief.companyName && brief.companyName.trim()) ? brief.companyName : 'je bedrijf';
  const subject = 'Je commercial-aanvraag is binnen bij TFA: ' + companyName;
...
// The dashboard's "Uitlevering" tab (see DashboardClient.js) — appears once
// the client has approved, lets a producer paste the final WAV Frame.io
// link plus a recipient email, and fires this the moment they hit send.
// Deliberately generic (not Advision-specific): the recipient is whoever
// the producer typed/picked, so the same flow works for a different agency
// or a client directly later without any code change. Reuses the same
// field table as the client confirmation/team-inbox emails
// (buildOverviewFields) so whoever receives this — an external production
// partner, an accountant, anyone — gets the full picture (script, voice,
// music) in one place rather than just a bare link, but with the music row
// narrowed to the one track actually used (see finalTrackOf in
// components/flowData.js) instead of all of the client's candidates.
function buildDeliveryEmailHtml(brief, deliveryLink) {
  const companyName = (brief.companyName && brief.companyName.trim()) ? brief.companyName : 'Onbekend bedrijf';
  const track = finalTrackOf(brief);
  const { rowsHtml, scriptBlock, variationBlocks } = buildOverviewFields(brief, { trackOverride: track });
  return (
    '<!doctype html><html lang="nl"><head><meta charset="utf-8"></head>' +
    '<body style="margin:0; padding:0; background:#DEDCD7; font-family:Arial, Helvetica, sans-serif;">' +
      '<div style="max-width:560px; margin:0 auto; padding:32px 20px;">' +
        '<div style="background:#FFFFFF; border-radius:16px; padding:36px 32px;">' +
          '<div style="font-size:11px; letter-spacing:.09em; text-transform:uppercase; color:#B08B00; font-weight:700;">TFA SpotFlow</div>' +
          '<h1 style="font-family:Georgia, serif; font-weight:600; font-size:22px; margin:14px 0 4px; color:#1D1D1D;">' + esc(companyName) + ': definitieve levering</h1>' +
          '<p style="font-size:14px; line-height:1.6; color:#5C5850; margin:0 0 20px;">De klant heeft dit traject goedgekeurd. Hieronder een overzicht van de productie; de definitieve WAV-bestanden staan klaar via de link onderaan.</p>' +
          '<table style="width:100%; border-collapse:collapse;">' + rowsHtml + '</table>' +
          scriptBlock +
          variationBlocks +
          '<div style="margin-top:22px;">' +
            '<a href="' + esc(deliveryLink || '') + '" style="display:inline-block; background:#E6C858; color:#1D1D1D; font-weight:700; font-size:14px; text-decoration:none; padding:13px 22px; border-radius:8px;">Open in Frame.io</a>' +
          '</div>' +
          '<p style="margin-top:18px; font-size:12px; color:#8C8880;">Met vriendelijke groet, TFA SpotFlow</p>' +
        '</div>' +
      '</div>' +
...
// Fires the moment a producer sends the "Uitlevering" tab — never throws
// (same philosophy as every other email here), so a bad recipient address
// or a Resend hiccup can't undo the link/recipient already having been
// saved on the brief by saveDelivery() in lib/db.js.
export async function sendDeliveryEmail(brief, deliveryLink, recipientEmail) {
  const companyName = (brief.companyName && brief.companyName.trim()) ? brief.companyName : 'Onbekend bedrijf';
  const to = (recipientEmail || '').trim();
  if (!to) return;
  const subject = 'Definitieve levering: ' + companyName;
  const html = buildDeliveryEmailHtml(brief, deliveryLink);
  if (RESEND_API_KEY) {
    try {
      await sendViaResend(to, subject, html);
      return;
    } catch (err) {
      console.error('[email] Delivery email via Resend failed, falling back to DB row:', err.message);
    }
  }
  try {
    await writeFallbackRow(brief.id, to, subject, html);
  } catch (err) {
    console.error('[email] Failed to save delivery-email fallback row:', err.message);
  }
}
