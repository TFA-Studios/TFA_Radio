...
import { variationsCountOf, TONE_LABELS, formatAirDate } from '../components/flowData';
...
// Exported so the dashboard's optional "Script bron" / "Impressies" columns
// (see the COLUMN configurator in DashboardClient.js) can reuse the exact
// same labels instead of a third copy of this mapping.
...
export const IMPRESSIONS_LABELS = {
...
export function buildReportData(briefs) {
  ...
  // Briefs whose client-requested air date has already passed and aren't
  // marked done yet — the same "overdue" definition (deliveryDeadlineMeta
  // in flowData.js) the dashboard table itself uses per row. Used to be
  // driven by a separately-typed-in internal deadline, but that never
  // reflected anything real since production always works as fast as
  // possible — the client's own date is the one that actually matters.
  const todayStart = new Date().setHours(0, 0, 0, 0);
  const overdueCount = briefs.filter((b) => {
    if (!b.airDate || b.dateUnknown || b.status === 'done') return false;
    const d = new Date(b.airDate + 'T00:00:00');
  ...
  const toneLinks = Object.keys(TONE_LABELS)
    .map((toneKey) => {
      ...
      return {
        ...
        topPlaylist: topPlaylist ? `${topPlaylist.label} (${topPlaylist.count}/${withTone.length})` : 'n.v.t.',
        topVoice: topVoice ? `${topVoice.label} (${topVoice.count}/${withTone.length})` : 'n.v.t.',
      };
    })
    .filter(Boolean)
    .sort((a, b) => b.count - a.count);
