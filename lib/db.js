...
// the round to react to. Also flips the producer-facing workflow `status`
// to 'pending_customer' — the same status already used everywhere else in
// the dashboard (stat tiles, filters, the table's Status column) to mean
// "waiting on the client" — so a review round in flight is visible without
// opening the brief at all, not just inside the Productie & review section.
...
async function pgAddReviewRound(id, { frameioLink }) {
  ...
  rounds.push({ id: nanoid(10), frameioLink: frameioLink || '', createdAt: new Date().toISOString(), feedback: [], approvedAt: null });
  const now = new Date().toISOString();
  await sql`UPDATE briefs SET "reviewRounds" = ${JSON.stringify(rounds)}, "productionStatus" = 'awaiting_review', "status" = 'pending_customer', "updatedAt" = ${now} WHERE id = ${id}`;
  return pgGetBrief(id);
}
