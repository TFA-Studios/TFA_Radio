...
async function memUpdateTeamMeta(id, { assignedTo, dueDate, seen }) {
  const row = mem.briefs.get(id);
  if (!row) return null;
  ...
  if (assignedTo !== undefined || dueDate !== undefined) row.updatedAt = new Date().toISOString();
  return rowToBrief(row);
}
...
async function pgUpdateTeamMeta(id, { assignedTo, dueDate, seen }) {
  await runMigrations();
  const { sql } = await import('@vercel/postgres');
  const existing = await sql`SELECT id FROM briefs WHERE id = ${id}`;
  if (!existing.rows.length) return null;
  ...
  if (assignedTo !== undefined || dueDate !== undefined) {
    sets.push(`"updatedAt" = $${i++}`);
    values.push(new Date().toISOString());
  }
  values.push(id);
