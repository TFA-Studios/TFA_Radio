...
function buildMainSentences(brief, targetWords) {
  ...
  // The call-to-action / website comes FIRST in the tail, immediately after
  // the body — it's the actual instruction to the listener ("ga naar
  // merk.nl") and reads best as the direct next step right after the
  // message, not buried before a closing brand moment. The payoff (slogan,
  // or at minimum the company name as a sign-off) comes AFTER the
  // call-to-action, as the true final spoken line before only the legal
  // disclaimer, if there is one — the brand name/slogan is only repeated
  // there if the client's own payoff text doesn't already contain it (see
  // sloganMentionsCompany below), so the brand never gets said twice in a
  // row. This is the reverse of the previous ordering (payoff/slogan, then
  // CTA), which Karim asked to flip.
  const tail = [];
...
function buildPrompt(brief, introText) {
  ...
  lines.push('5. De call-to-action of website, direct na de kernboodschap. Dit is VERPLICHT als de brief hem geeft, laat hem nooit weg.');
  lines.push('6. Merkafsluiting: de bedrijfsnaam, gevolgd door de slogan exact zoals aangeleverd, als allerlaatste gesproken regel vóór een eventuele disclaimer. Dit is VERPLICHT als er een slogan is. Als de slogan (of het ontbreken ervan) de bedrijfsnaam al bevat, herhaal die naam er niet nog eens vlak voor: noem de merknaam maar één keer in deze afsluiting. BELANGRIJK: let ook op de website zelf uit stap 5: bevat die (bijv. "tfa.studio") de bedrijfsnaam al ("TFA" zit al in de domeinnaam), herhaal de bedrijfsnaam dan hier niet nog eens los. Dus niet "Kijk op TFA.studio. TFA." (dat klinkt als een stotterende herhaling), maar bijvoorbeeld gewoon de slogan alleen, of de merknaam op een andere, natuurlijke manier die niet direct tegen de website-naam aan botst.');
  if (hasDisclaimer) {
    lines.push('7. Helemaal aan het eind, als allerlaatste zin(nen), na de merkafsluiting: de verplichte tekst/disclaimer, WOORDELIJK en ONGEWIJZIGD overgenomen uit de brief. Dit is wettelijk verplichte tekst, herschrijf, parafraseer of verkort hem nooit.');
  }
  ...
  if (tones.length) {
    lines.push('- De gekozen toon (' + tones.map((t) => TONE_LABELS[t]).join(' & ') + ') is LEIDEND voor woordkeuze, ritme en zinsbouw: belangrijker dan enige standaard "commercial-stijl" hierboven. Als de gekozen toon en de standaardstijl uit elkaar lopen (bijv. een luchtige of drooghumoristische toon versus een neutrale "nieuwsradio"-inleiding), kies dan voor de gekozen toon: het script moet duidelijk anders klinken dan een script zonder tonekeuze, of met een andere toon. Laat dit door het HELE script doorklinken, niet alleen in de opening.');
    if (tones.length > 1) {
