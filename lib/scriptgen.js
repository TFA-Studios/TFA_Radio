...
const TONE_GUIDE = {
  ...
  inspirerend: 'Taal die een groter perspectief schetst dan alleen het product — wat het mogelijk maakt voor de luisteraar, bouwend naar een motiverende afsluiting.',
...
function buildPrompt(brief, introText) {
  ...
  lines.push('6. De call-to-action of website, als allerlaatste gesproken regel vóór een eventuele disclaimer. Dit is VERPLICHT als de brief hem geeft — laat hem nooit weg. BELANGRIJK: dit is het echte einde van het script — noem na de call-to-action NIET nog een keer de bedrijfsnaam of iets uit stap 5, ook niet in een andere vorm. Eén keer de bedrijfsnaam/slogan noemen (stap 5) is genoeg; herhaal die niet als afsluitende zin ná de call-to-action.');
  if (hasDisclaimer) {
