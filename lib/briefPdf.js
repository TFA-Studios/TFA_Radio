...
import { variationsCountOf, variationsSummaryLabel, formatAirDate, parseVariationScripts, ipRightsDisclaimerText, agencyNameOf } from '../components/flowData';
...
function field(doc, label, value) {
  ...
  doc.font('Helvetica').fontSize(9).fillColor(value ? INK : FAINT).text(' ' + (value || 'n.v.t.'));
}
...
export function buildBriefPdf(brief) {
  return new Promise((resolve, reject) => {
    ...
    additionalContacts.forEach((c, i) => {
      field(doc, 'Extra contact' + (additionalContacts.length > 1 ? ' ' + (i + 1) : ''), c.name && c.email ? c.name + ' (' + c.email + ')' : [c.name, c.email].filter(Boolean).join(''));
    });
    ...
    if (disclaimerText) {
      sectionTitle(doc, 'Verplichte tekst / disclaimer');
      doc.font('Helvetica').fontSize(10).fillColor(INK).text(disclaimerText, { lineGap: 2 });
    }

    // Same fixed terms as the "Voorwaarden" box on the client's overview
    // step (app/brief/[id]/overview/page.js) and the confirmation email
    // (lib/email.js's buildTermsBlock) — kept here too so the producer's
    // downloadable PDF is a complete written record of what the client
    // agreed to, not just the creative choices.
    sectionTitle(doc, 'Voorwaarden');
    doc.font('Helvetica').fontSize(9).fillColor(MUTED).list([
    ...
    ], { lineGap: 2, bulletRadius: 1.5 });

    sectionTitle(doc, 'Status');
    field(doc, 'Agentschap', agencyNameOf(brief));
