...
import { variationsSummaryLabel, TONE_LABELS } from '../components/flowData';
...
function buildOverviewFields(brief) {
  ...
  return { rowsHtml, scriptBlock, disclaimerBlock };
}
...
function buildEmailHtml(brief, appUrl) {
  ...
  const { rowsHtml: fieldsHtml, scriptBlock, disclaimerBlock } = buildOverviewFields(brief);
  const rowsHtml = '<tr>' +
...
function buildTeamNotificationHtml(brief, dashboardUrl) {
  ...
  const { rowsHtml, scriptBlock, disclaimerBlock } = buildOverviewFields(brief);
  return (
    '<!doctype html><html lang="nl"><head><meta charset="utf-8"></head>' +
    '<body style="margin:0; padding:0; background:#DEDCD7; font-family:Arial, Helvetica, sans-serif;">' +
      '<div style="max-width:560px; margin:0 auto; padding:32px 20px;">' +
